...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BRG")
    updateDisplayUnit();
}
...
{
    QString ret = QString("borgcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BRG unit input in lnReqAmount, we generate the URI with BRG as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BRG, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
